...
class WichtelManager {
	...
	public function shuffle() {
		...
		foreach ($this->wichtel as $act) {
			$w = $this->getRandomFreeWichtelFromOtherGroup($act,$act->gruppe);
	...
	public function printWichtel() {
		...
		foreach($this->wichtel as $w) {
			if ($w->wichtel instanceof Wichtel) {
				$this->out("{$w->name}'s Wichtel is: {$w->wichtel->name}.".($w->gruppe == $w->wichtel->gruppe?' (!! SAME GROUP !!)':'')."\n");
			} else {
	...
	private function getRandomFreeWichtelFromOtherGroup($checkWichtel, $group) {
		...
		// find available:
		foreach ($this->wichtel as $w) {
			if ($w != $checkWichtel && $w->gruppe != $group && $w->wichtelOf == null)
				$available[] = $w;
		}
